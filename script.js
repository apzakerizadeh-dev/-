...
document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('شماره فرم فعلی:', getNextFormNumber());
});
...
// بارگذاری شماره فرم از localStorage
function loadFormNumber() {
    const savedNumber = localStorage.getItem('deliveryFormNumber');
    if (savedNumber) {
        formNumber = parseInt(savedNumber);
    } else {
        formNumber = 1;
        saveFormNumber();
    }
...
// ذخیره شماره فرم در localStorage
function saveFormNumber() {
    localStorage.setItem('deliveryFormNumber', formNumber.toString());
}

// تولید شماره فرم جدید
function generateFormNumber() {
    formNumber++;
    saveFormNumber();
    return formNumber - 1; // شماره فرم فعلی را برمی‌گرداند
}
...
function generateSVG() {
    // افزایش شماره فرم
    const formNumber = getNextFormNumber(); // این شماره فرم را افزایش می‌دهد

    // جمع‌آوری داده‌های فرم
    const formData = collectFormData();
    formData.formNumber = formNumber - 1; // استفاده از شماره فرم قبلی
    ...
    // نمایش پیام موفقیت
    showMessage('✅ فایل SVG با موفقیت تولید شد! می‌توانید آن را دانلود کنید.', 'success');
}
...
function showPreview(svgContent) {
    ...
    // اضافه کردن SVG به صفحه
    svgPreview.innerHTML = svgContent;
    
    // اسکرول به بخش پیش‌نمایش
    previewSection.scrollIntoView({ behavior: 'smooth' });
    
    showMessage('پیش‌نمایش فایل تولید شده آماده است', 'info');
}
...
function downloadSVG() {
    ...
    showMessage('✅ فایل با موفقیت دانلود شد: ' + filename, 'success');
}
...
function shareSVG() {
    ...
    if (navigator.share) {
        ...
        navigator.share({
            ...
            showMessage('✅ فایل با موفقیت به اشتراک گذاشته شد', 'success');
        }).catch(error => {
...
function copySVG() {
    ...
    navigator.clipboard.writeText(currentSvgContent).then(() => {
        showMessage('✅ متن SVG با موفقیت کپی شد', 'success');
    }).catch(err => {
        showMessage('❌ خطا در کپی کردن متن. لطفاً مرورگر خود را بررسی کنید.', 'danger');
...
function resetForm() {
    ...
    if (resetChoice === true) {
        ...
        if (confirm('آیا از پاک کردن تمام اطلاعات فرم فعلی اطمینان دارید؟')) {
            ...
            // پاک کردن ذخیره‌سازی محلی فرم (ولی شماره فرم باقی می‌ماند)
...
// در initializeApp() این کد را اضافه کنید:
function initializeApp() {
